...
namespace seneca {
    ...
    std::ostream& Publication::write(std::ostream& os) const {
        if (conIO(os)) {
            ...
            std::string title = m_title ? m_title : "";
            if (title.length() > SENECA_TITLE_WIDTH) {
                title = title.substr(0, SENECA_TITLE_WIDTH);
            }

            os << "| " << std::setw(SENECA_SHELF_ID_LEN) << m_shelfId << " | "
                << std::left << std::setw(SENECA_TITLE_WIDTH) << std::setfill('.') << title << " | "
